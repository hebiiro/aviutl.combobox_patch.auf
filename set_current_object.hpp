...
//
inline struct SetCurrentObject : Addin
{
	//
	// コンストラクタです。
	//
	...
	inline static void CDECL hook_proc(int32_t object_index)
	{
		...
		{
			output_debug_string("combobox_index = {}", combobox_index);
			...
			if (filter.is_animation_effect())
			{
				...
				output_debug_string("アニメーション効果 : {}, {:#010x}, {}, {}",
				...
				if (filter_category_id != cached_category_id)
				{
					...
					{
						output_debug_string("cached : {} <=> {}", combobox_index, cached_combobox_index);
			...
			for (int32_t j = rest; j < check_n; j++)
			{
				...
				output_debug_string("残りのコンボボックス : {}, {:#010x}, {}",
				...
				if (cached_category_id)
				{
					...
					{
						output_debug_string("nocached : {} <=> {}", rest_combobox_index, nocached_combobox_index);
	...
	virtual BOOL func_init(AviUtl::FilterPlugin* fp) override
	{
		...
		orig_proc = decltype(orig_proc)(exin::exedit + 0x305E0);
